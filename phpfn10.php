...
function CurrentProjectID() {
	...
	return "{C2A46AD1-29DD-4049-B347-17989E75216E}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('userid')); // Load user name
						$this->setSessionUserID($rs->fields('site')); // Load User ID
						$this->setSessionParentUserID($rs->fields('site')); // Load parent User ID
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('site'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`site` IN (' . $sCurUserIDList . ')';
					$sSql = $user->GetSQL($sFilter, '');
					if ($RsUser = $conn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('site'));
	...
	function UserIDList() {
		...
		$len = count($ar);
		for ($i = 0; $i < $len; $i++)
			$ar[$i] =  ew_QuotedValue($ar[$i], EW_DATATYPE_STRING);
		return implode(", ", $ar);
	}
	...
	function ParentUserIDList($userid) {
		...
		if (trim(strval($userid)) == strval(CurrentUserID())) {
			if (strval(CurrentParentUserID()) <> "")
				$result = ew_QuotedValue(CurrentParentUserID(), EW_DATATYPE_STRING);
			return $result;
		}

		// One level only, must be CurrentUserID
		if (!EW_USER_ID_IS_HIERARCHICAL) {
			return ew_QuotedValue(CurrentUserID(), EW_DATATYPE_STRING);
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) <> trim(strval($userid))) {
					if ($result <> "")
						$result .= ", ";
					$result .= ew_QuotedValue($ar[$i], EW_DATATYPE_STRING);
				}
			}
			return $result;
		}
