	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin
				if (strval($this->site->CurrentValue) == strval(CurrentUserID())) {
			if (strval($this->site->CurrentValue) <> "") {
				switch ($this->site->CurrentValue) {
					case $this->site->FldTagValue(1):
						$this->site->EditValue = $this->site->FldTagCaption(1) <> "" ? $this->site->FldTagCaption(1) : $this->site->CurrentValue;
						break;
					case $this->site->FldTagValue(2):
						$this->site->EditValue = $this->site->FldTagCaption(2) <> "" ? $this->site->FldTagCaption(2) : $this->site->CurrentValue;
						break;
					default:
			...
			$sFilterWrk = "";
			$sFilterWrk = $GLOBALS["user"]->AddParentUserIDFilter("", $this->site->CurrentValue);
...
fuseredit.Lists["x_UserLevel"] = {"LinkField":"x_userlevelid","Ajax":null,"AutoFill":false,"DisplayFields":["x_userlevelname","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fuseredit" id="fuseredit" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<span id="el_user_id" class="control-group">
...
<?php echo $user->id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($user->_userid->Visible) { // userid ?>
	<tr id="r__userid">
		<td><span id="elh_user__userid"><?php echo $user->_userid->FldCaption() ?></span></td>
		<td<?php echo $user->_userid->CellAttributes() ?>>
<span id="el_user__userid" class="control-group">
<input type="text" data-field="x__userid" name="x__userid" id="x__userid" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($user->_userid->PlaceHolder) ?>" value="<?php echo $user->_userid->EditValue ?>"<?php echo $user->_userid->EditAttributes() ?>>
</span>
<?php echo $user->_userid->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_user_password" class="control-group">
<input type="password" data-field="x_password" name="x_password" id="x_password" value="<?php echo $user->password->EditValue ?>" size="30" maxlength="100"<?php echo $user->password->EditAttributes() ?>>
</span>
<?php echo $user->password->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($user->site->Visible) { // site ?>
	<tr id="r_site">
		<td><span id="elh_user_site"><?php echo $user->site->FldCaption() ?></span></td>
		<td<?php echo $user->site->CellAttributes() ?>>
<?php if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin ?>
<?php if (strval($user->site->CurrentValue) == strval(CurrentUserID())) { ?>
<span id="el_user_site" class="control-group">
<span<?php echo $user->site->ViewAttributes() ?>>
<?php echo $user->site->EditValue ?></span>
</span>
<input type="hidden" data-field="x_site" name="x_site" id="x_site" value="<?php echo ew_HtmlEncode($user->site->CurrentValue) ?>">
<?php } else { ?>
<span id="el_user_site" class="control-group">
<select data-field="x_site" id="x_site" name="x_site"<?php echo $user->site->EditAttributes() ?>>
<?php
if (is_array($user->site->EditValue)) {
	$arwrk = $user->site->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($user->site->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php } ?>
<?php } elseif (!$Security->IsAdmin() && $Security->IsLoggedIn() && !$user->UserIDAllow("edit")) { // Non system admin ?>
<span id="el_user_site" class="control-group">
<select data-field="x_site" id="x_site" name="x_site"<?php echo $user->site->EditAttributes() ?>>
<?php
if (is_array($user->site->EditValue)) {
...
<?php } else { ?>
<span id="el_user_site" class="control-group">
<div id="tp_x_site" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><input type="radio" name="x_site" id="x_site" value="{value}"<?php echo $user->site->EditAttributes() ?>></div>
<div id="dsl_x_site" data-repeatcolumn="5" class="ewItemList">
<?php
$arwrk = $user->site->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($user->site->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label class="radio"><input type="radio" data-field="x_site" name="x_site" id="x_site_<?php echo $rowcntwrk ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $user->site->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
<?php } ?>
<?php echo $user->site->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin ?>
<span id="el_user_UserLevel" class="control-group">
<?php echo $user->UserLevel->EditValue ?>
</span>
<?php } else { ?>
<span id="el_user_UserLevel" class="control-group">
...
<script type="text/javascript">
fuseredit.Lists["x_UserLevel"].Options = <?php echo (is_array($user->UserLevel->EditValue)) ? ew_ArrayToJson($user->UserLevel->EditValue, 1) : "[]" ?>;
</script>
</span>
<?php } ?>
<?php echo $user->UserLevel->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<span id="el_user__email" class="control-group">
<input type="text" data-field="x__email" name="x__email" id="x__email" size="30" maxlength="100" placeholder="<?php echo ew_HtmlEncode($user->_email->PlaceHolder) ?>" value="<?php echo $user->_email->EditValue ?>"<?php echo $user->_email->EditAttributes() ?>>
</span>
<?php echo $user->_email->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
