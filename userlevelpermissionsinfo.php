...
class cuserlevelpermissions extends cTable {
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			...
			if (array_key_exists('tablename', $rs))
				ew_AddFilter($where, ew_QuotedName('tablename') . '=' . ew_QuotedValue($rs['tablename'], $this->_tablename->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("userlevelpermissionsview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("userlevelpermissionsview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
